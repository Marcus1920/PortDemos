...
class UserController extends Controller {
	...
	public function getPoisAssociates($id, $responseJson = true) {
		$txtDebug = __CLASS__."".__FUNCTION__."(\$id, \$responseJson)";
		$response             = array();
		$responseNodes        = array();
		$responseLinks        = array();
	...
	public function main_poi_cases_associate($poi_id, $responseNodes, $responseLinks) {
		...
		$txtDebug .= "\n  \$poi - ".print_r($poi,1);
		$txtDebug .= "\n  \$poipicture - ".print_r($poipicture,1);
		//die("<pre>{$txtDebug}</pre>");
		if (sizeof($poi) > 0) {
			$userObj          = new \stdClass();
			$userObj->id      = "m-" . $poi->id;
			$userObj->name    = $poi->name . " " . $poi->surname;
			$userObj->picture = $poipicture ? $poipicture->poi_picture_url : "images/poi/profile/no_photo.png";
			$userObj->type    = "MAIN";
			$userObj->loaded  = true;
	...
	public function main_associate($id, $responseNodes, $responseLinks) {
		...
		if (sizeof($poi) > 0) {
			...
			$userObj->id      = "m-" . $poi->id;
			$userObj->name    = $poi->name . " " . $poi->surname;
			$userObj->picture = $poipicture && $poipicture->poi_picture_url != "" && file_exists($poipicture->poi_picture_url) ? $poipicture->poi_picture_url : "images/poi/profile/no_photo.png";
			$userObj->type    = "MAIN";
			$userObj->loaded  = true;
			$responseNodes[]  = $userObj;
			$main["nodes"]    = $responseNodes;
...
//die("<pre>{$txtDebug}</pre>");
		...
		if ($poiObj && $poipicture ) {
			//$userObj->picture = $poipicture->poi_picture_url;
			if (file_exists($poipicture->poi_picture_url)) $userObj->picture = $poipicture->poi_picture_url;
			else $userObj->picture = "images/poi/profile/photo_notfound.png";
		}
		else $userObj->picture = "images/poi/profile/no_photo.png";
	...
	public function store(UserRequest $request, User $user) {
		...
		$user->cellphone                   = str_replace(" ", "", $request['cellphone']);
		$user->alt_cellphone               = str_replace(" ", "", $request['alt_cellphone']);
	...
	public function ak_img_resize($target, $newcopy, $w, $h, $ext) {
		...
		$tci = imagecreatetruecolor($w, $h); // imagecopyresampled(dst_img, src_img, dst_x, dst_y, src_x, src_y, dst_w, dst_h, src_w, src_h)
			...
			->select(
				\DB::raw(
					"
                                         ...
                                         poi.tax_number,
                                         poi.religion,
                                         poi.qualification_type,
                                         poi.notice_period,
                                         poi.doc_expiry_date,
                                         poi.work_permit,
                                         poi.work_permit_expiry_date,
                                         poi.yellow_fever,
                                         poi.yellow_fever_expiry_date,
	...
	public function view_poi_associates($id) {
		...
		if ($poi__associate_query > 0) {
			...
			foreach ($associates as $associate) {
				...
				//die("<pre>{$txtDebug}</pre>");
				$txtDebug .= "\n  Setting picture for: id - {$associate->id}";
				if ($assoc_poi_object) $assoc_poi_object->poi_picture_url = $assoc_pic_object && file_exists($assoc_pic_object->poi_picture_url) ? $assoc_pic_object->poi_picture_url : "images/poi/profile/no_photo.png";
				$sub_results = array();
				//Sub - Associates
	...
	public function view_case_poi_associates($case_id) {
		$txtDebug = __CLASS__ . "." . __FUNCTION__ . "(\$case_id) \$case_id - {$case_id}";
		$case = CaseReport::find($case_id);
		$txtDebug .= PHP_EOL . "  \$case - " . print_r($case, 1);
	...
	public function view_poi_cases_associates($poi_id) {
		$txtDebug = __CLASS__ . "." . __FUNCTION__ . "(\$poi_id) \$poi_id - {$poi_id}";
		$poi = Poi::find($poi_id);
	...
	public function edit($id, User $user) {
		...
		if ($userObj->affiliation > 0) {
			...
			$user->affiliation = $affiliation ? $affiliation->name : "unknown";
		}
		...
		if ($userObj->position > 0) {
			...
			$user->position = $position ? $position->slug : "";
		}
		if ($userObj->title > 0) {
	...
	public function update(UpdateUserRequest $request) {
		...
		$txtDebug .= "\n  Here A";
		if (array_key_exists("affiliation", $request) && $request['affiliation'] == "Select / All") {
			$users_if = 1;
		}
		else if (array_key_exists("affiliation", $request)) {
			$Affiliation_id = Affiliation::where('name', $request['affiliation'])->first();
			$users_if       = $Affiliation_id->id;
		}
		$txtDebug .= "\n  Here B";
		$user                              = User::where('id', $request['userID'])->first();
		//echo "\n  Here C, \$user - ".print_r($user,1);
		$txtDebug .= "\n  \$user (A): ".print_r($user->toArray(),1);
		$role                              = UserRole::where('slug', '=', $request['role'])->first();
		$txtDebug .= "\n  Here D";
		$user->role                        = $role ? $role->id : 0;
		$txtDebug .= "\n  Here E";
		...
		$user->surname                     = $request['surname'];
		$user->id_number                   = $request['id_number'];
		$codes = \App\Providers\FormFieldProvider::getDialingCodes();
		$codes = array_keys($codes);
		asort($codes, SORT_DESC);
		$txtDebug .= "\n  \$codes - ".print_r($codes,1);
		$user->cellphone                   = str_replace(" ", "", $request['cellphone']);
		$user->alt_cellphone               = str_replace(" ", "", $request['alt_cellphone']);
